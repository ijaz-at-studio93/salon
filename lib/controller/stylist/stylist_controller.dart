...
class StylistController extends GetxController {
  ...
  final Rx<PendingAppointmentsListModel> _pendingAppointmentsListModel = PendingAppointmentsListModel().obs;
  PendingAppointmentsListModel get getPendingAppointmentsListModel => _pendingAppointmentsListModel.value;
  set setPendingAppointmentsListModel(val) => _pendingAppointmentsListModel.value = val;

  /*----------------- Accept Booking ---------------------------*/
  final Rx<PendingAppointmentsListModel> _acceptAppointmentsListModel = PendingAppointmentsListModel().obs;
  PendingAppointmentsListModel get getAcceptAppointmentsListModel => _acceptAppointmentsListModel.value;
  set setAcceptAppointmentsListModel(val) => _acceptAppointmentsListModel.value = val;
  ...
  doBookingApprove({required String appointmentId, required String status, required VoidCallback callback}) async{
    try{
      _showProgress.value = true;
      bool result = await StylistAPI.approveBooking(appointmentId: appointmentId, status: status);
      if(result){
        callback.call();
      }
    }catch(e){
